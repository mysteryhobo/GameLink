...
/** The KalahRunner classes starts both KalahAutoPlayer classes so that they mey simulate the playing of player 1 and player 2 for all games in the tournament */
public class KalahTournamentSimulator{
...
/**
* The Player1Simulator class acts as player 1 in the tournament by using the algorithm of the team it is simulating. after the completion of the tournament it writes the results
...
class Player1Simulator extends Thread{
	...
	public void run(){
		...
		System.out.println("********************************************************************************");
		System.out.println("NEW KALAH TOURNAMENT: TEAMS: " + teamList.size() + ", MATCHES: " + (matchList.size() / 2));
		...
		try {
			...
			while(matchIterator.hasNext()) {
				...
				Kalah game = new Kalah(PLAYER1, true, display);
	...
	protected static String generateUniqueFileName(){
        ...
        return "Kalah-Results:" + filename.substring(0,19);
	}
	 ...
	 */
	private void printLeaderBoard(ArrayList<Team> teamList, PrintWriter pw){
	...
	private String formatName(String name) {
		...
		int errorLength = name.length() - MAX_NAME_LENGTH;
	...
	*/
	public void run(){
		...
		while(matchIterator.hasNext()){
			...
			Kalah game;
			if (!distributedMode) {game = new Kalah(PLAYER2, true, display);}
			else game = new Kalah(ipAddressOfP1Simulator, display);
	...
	public static Class<?> getAlgorithm(Match currMatch, ArrayList<Class<?>> algoList, int player) {
		for (Class<?> algo : algoList){
			try{
				...
				if (player == 1) {
					if (getTeamName.invoke(new Object[] {null}).equals(currMatch.getTeam1Name())) return algo;
				} else {
					if (getTeamName.invoke(new Object[] {null}).equals(currMatch.getTeam2Name())) return algo;
				}
